    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
          <h2 className="text-xl font-bold text-navy">{devotional ? "Editar Devocional" : "Nuevo Devocional"}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <Input label="Título" name="title" defaultValue={devotional?.title} required />
          
          <div className="grid grid-cols-2 gap-4">
            <Input label="Autor" name="author" defaultValue={devotional?.author || "Equipo Pastoral ICCI"} required />
          ...
          <div>
            <label className="block text-sm font-semibold text-navy mb-2">Reflexión</label>
            <textarea 
              name="content" 
          ...
          <div>
            <label className="block text-sm font-semibold text-navy mb-2">
              Fotografía de Portada {devotional ? "(Opcional si no la cambias)" : "(Opcional)"}
            </label>
            <input 
              type="file" 
              name="image" 
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button type="submit" disabled={loading} className="flex-1 flex items-center justify-center gap-2">
              ...
              {devotional ? "Guardar Cambios" : "Publicar Devocional"}
            </Button>
          </div>
        </form>
      </div>
